...
const JobCard = (props: IProps) => {
    const { showPagination = false, searchText = "" } = props;
    ...
    const searchTextHome = (new URLSearchParams(location.search).get('searchText') || "");
    const navigate = useNavigate();
    useEffect(() => {
        fetchJob();
    }, [current, pageSize, filter, sortQuery, searchText, searchTextHome]);

    const fetchJob = async () => {
        setIsLoading(true)
        let query = searchText ? `current=${current}&pageSize=${pageSize}&${searchText}` : `current=${current}&pageSize=${pageSize}&${searchTextHome}`;
        if (filter) {
            query += `&${filter}`;
        }
        if (sortQuery) {
